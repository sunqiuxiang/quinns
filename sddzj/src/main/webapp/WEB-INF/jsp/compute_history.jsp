...
<body class="pos-r">
	...
	<div>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 平台服务 <span class="c-gray en">&gt;</span> 历史申请 <span class="c-gray en">&gt;</span> 互相关计算<a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
<div align="center"><a href="<%=request.getContextPath() %>/data/compute"><span style="font-size:18px">互相关申请</span></a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
<a href="<%=request.getContextPath()%>/data/computeApply"><b style="font-size:19px;text-decoration:underline">历史申请</b></a></div>
<div class="page-container">
  <table class="table table-border table-bordered table-bg table-hover table-sort">
    <thead>
      <tr class="text-c">
     		<th scope="col" colspan="9">历史申请</th>
			</tr>
			<tr class="text-c">
				...
				<th width="160">审批时间</th>
				<!-- <th width="100">muban</th> -->
				<th width="100">MR任务</th>
				<!-- <th width="100">MR下载</th> -->
				<th width="100">Spark任务</th>
				<!-- <th width="100">Spark下载</th> -->
      </tr>
    </thead>
    <tbody>
     <c:forEach items="${tasks}" var="task">
                  <tr>
                        ...
                        <td>${task.checktime }</td> 
                       <%--  <td>${task.muban} </td> --%>
                       <c:if test="${task.reviewed =='0' }"><td>待审核</td></c:if>
                        <c:if test="${task.reviewed =='2' }"><td>未同意</td></c:if>
                         <c:if test="${task.status==null&&task.reviewed !='2' }"><td>进行中</td></c:if>
                        <c:if test="${task.taskStatus=='success' }">
                        <c:if test="${task.reviewed !='4' }">
                        <td><a href="<%=request.getContextPath()%>/data/download/compute_result.do?task_id=${task.id}&task_type=t_task_result">下载</a></td> 
                        </c:if>
                         <c:if test="${task.reviewed =='4' }">
                        <td>已过期</td> 
                        </c:if>
                        </c:if>
                        <c:if test="${task.taskStatus=='failure'&&task.reviewed !='2' }"><td>失败</td></c:if>
                        
                        <c:if test="${task.reviewed =='2' }"><td>未同意</td></c:if>
                         <c:if test="${task.taskStatusSpark==null&&task.reviewed !='2' }"><td>进行中</td></c:if>
                        <c:if test="${task.taskStatusSpark=='success' }">
                        <c:if test="${task.reviewed !='4' }">
                        <td><a href="<%=request.getContextPath()%>/data/download/compute_result?task_id=${task.id}&task_type=t_task_result_spark">下载</a></td> 
                        </c:if>
                         <c:if test="${task.reviewed =='4' }">
                        <td>已过期</td> 
                        </c:if>
                        </c:if>
                        <c:if test="${task.taskStatusSpark=='failure'&&task.reviewed !='2' }"><td>失败</td></c:if>
                    </tr> 
                </c:forEach>
    </tbody>
  </table>
