...
<div class="page-container">
	<div class="text-c">
		<form action="<%=request.getContextPath() %>/file/search.do" method="post">
			<input type="hidden" name="path" value="/user/hdfs"> 
	...
	<div class="cl pd-5 bg-1 bk-gray mt-20"> 
		<span class="l">
			...
			<a class="btn btn-primary radius" onclick="mkdir()" href="javascript:;">
				<i class="Hui-iconfont">&#xe600;</i> 创建文件夹
			</a>
			<%-- <%=request.getContextPath() %>/file/batchdownload.do --%>
			...
			<a href="${pageContext.request.contextPath }/file/list?filePath=${fileParent}" class="btn btn-primary radius">
				<i class="Hui-iconfont"></i>返回上一级
			</a>  
		</span>
	</div>
	<div class="file_nav"></div>
			...
			<tbody id="file_table">
				<c:if test="${list != null }">
					<c:forEach var="file" items="${list }">
						...
						<c:if test="${file.dir }">
							<td class="name"><span class="edit_part hidden_part"><input value="${file.fileName }"/><img src="<%=request.getContextPath()%>/images/img_btn_ok.png" class="img_btn img_btn_ok" onclick=""/><img src="<%=request.getContextPath()%>/images/img_btn_cancel.png" class="img_btn img_btn_cancel" onclick=""/></span><a class="filename_part" href="${pageContext.request.contextPath }/file/list.do?filePath=${file.filePath}" onClick=""><input type="hidden" value="${file.filePath }"/><img src="<%=request.getContextPath()%>/images/folderClosed.gif" width="18px" height="18px"/>${file.fileName }</a></td>
							<td>-</td>
						</c:if>
						<c:if test="${!file.dir }">
							<td class="name"><span class="edit_part hidden_part"><input value="${file.fileName }"/><img src="<%=request.getContextPath()%>/images/img_btn_ok.png" class="img_btn img_btn_ok" onclick=""/><img src="<%=request.getContextPath()%>/images/img_btn_cancel.png" class="img_btn img_btn_cancel" onclick=""/></span><a class="filename_part" href="${pageContext.request.contextPath }/file/download.do?filePath=${file.filePath}" onClick=""><input type="hidden" value="${file.filePath }"/><img src="<%=request.getContextPath()%>/images/leaf.gif" width="18px" height="18px"/>${file.fileName }</a></td>
							<td>${file.size }</td>
						</c:if>
...
/*文件重命名:不要轻易动这个方法*/
	...
	$($($(obj).parents('tr').find(".edit_part")[0]).find(".img_btn_ok")[0]).click(function(){
		...
		$.ajax({
			...
			success:function(resultData){
				var resultData = eval("("+resultData+")");
				if(resultData.success){
					$(obj).parents('tr').find(".filename_part")[0].innerHTML = isDir == 'true'?"<img src='<%=request.getContextPath()%>/images/folderClosed.gif' width='18px' height='18px'/>"+newname:"<img src='<%=request.getContextPath()%>/images/leaf.gif' width='18px' height='18px'/>"+newname;
 ...
 function mkdir(){
	 var elems = "<tr class='text-c'>"+
	 			   ...
	 			   "</td>"+
	 				"<td class='name'>"+
	 				"<img src='<%=request.getContextPath()%>/images/folderClosed.gif' width='18px' height='18px'/><input class='input_mkdir' value='mkdir'/><img src='<%=request.getContextPath()%>/images/img_btn_ok.png' class='img_btn img_btn_ok' onclick=''/><img src='<%=request.getContextPath()%>/images/img_btn_cancel.png' class='img_btn img_btn_cancel' onclick=''/>"+
	 			   "</td>"+
				   "<td>-</td>"+
				  "<td>-</td>"+
				  "<td>-</td>"+
	 ...
	 $($(".img_btn_ok")[0]).click(function(){
		 $.ajax({
			 url:'<%=request.getContextPath() %>/file/mkdir',
 ...
 function batchDelete(){
	 ...
	 if(checked.length >= 1){
		 ...
		 $.ajax({
			 url:'<%=request.getContextPath() %>/file/batchdelete',
