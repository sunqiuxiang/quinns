...
<body>
	...
	<div>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 平台服务 <span class="c-gray en">&gt;</span> 下载申请<span class="c-gray en">&gt;</span> ${list3 } <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
<div align="center"><a href="<%=request.getContextPath()%>/data/chaxunxiazai?tw_name=NetName&taizhen=${list4}"><span style="font-size:18px">下载申请</span></a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
<a href="<%=request.getContextPath()%>/data/xiazailishi?taizhen=${list4}"><b style="font-size:19px;text-decoration:underline">历史申请</b></a></div>
...
<div class="page-container">
  ...
  <table class="table table-border table-bordered table-bg table-hover table-sort">
    <thead>
     <tr>
				<th scope="col" colspan="10" align="center">历史申请</th>
			</tr>
			<tr class="text-c">
     	...
     	<c:forEach items="${list}" var="user">
                  <tr>
                  	<!-- <td><input type="checkbox" value="" name=""></td> -->
                        ...
                        <c:if test="${user.reviewed=='2'&&(user.allfilepath == null || user.allfilepath == '') }"><td>拒绝</td></c:if>
                        <c:if test="${user.reviewed=='2'&&(user.allfilepath != null && user.allfilepath != '') }"><td>同意</td></c:if>
                        
                        
                        <c:if test="${user.reviewed =='3'||(user.allfilepath != null && user.allfilepath != ''&&user.reviewed !='4' ) }">
                        	<td><a href="<%=request.getContextPath() %>/upload/download?task_id=${user.id }&filepath=${user.allfilepath }">全部下载</a></td> 
                        </c:if>
                         <c:if test="${(user.allfilepath == null||user.allfilepath == '') && user.reviewed =='1' && user.flag=='1' }">
                         <td>处理中</td> 
                         </c:if>
                          <c:if test="${(user.allfilepath == null||user.allfilepath == '') && user.reviewed =='0' && user.flag=='1'  }">
                         <td>未处理</td> 
                         </c:if>
                          <c:if test="${(user.allfilepath == null||user.allfilepath == '') && user.reviewed =='2' && user.flag=='1'  }">
                         <td>已拒绝</td> 
                         </c:if>
                         <c:if test="${(user.allfilepath == null||user.allfilepath == '') && user.flag=='0'  }">
                         <td>未压缩</td> 
                         </c:if>
                         <c:if test="${user.reviewed =='4' }">
                         <td>已过期</td> 
                         </c:if>
                         <td><a href="<%=request.getContextPath() %>/data/particular?applyname=${user.applyname }&applytime=${user.applytime }&taizhen=${list4}">详情</a></td>
                    </tr> 
                </c:forEach>
